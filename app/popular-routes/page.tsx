    ...
    <main>
      ...
      <section className="flex items-center justify-center px-8 py-24 text-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #071e27 0%, #0d3349 50%, #00639a 100%)', minHeight: '44vh' }}>
        ...
        <div className="relative z-10 space-y-5 animate-entrance opacity-0" style={{ animationFillMode: 'forwards' }}>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20">
            <span className="material-symbols-outlined text-blue-300 text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>route</span>
      ...
      <section className="py-10 px-6 max-w-screen-xl mx-auto">
        <div className="flex flex-wrap gap-3 justify-center">
          <button className={`px-5 py-2 rounded-full font-headline font-bold text-sm border-2 transition-all ${filter === 'all' ? 'bg-emerald-800 border-emerald-800 text-white' : 'border-slate-300 bg-white text-slate-600 hover:border-emerald-800 hover:text-emerald-800'}`} onClick={() => setFilter('all')}>All Routes</button>
          <button className={`px-5 py-2 rounded-full font-headline font-bold text-sm border-2 transition-all ${filter === 'popular' ? 'bg-emerald-800 border-emerald-800 text-white' : 'border-slate-300 bg-white text-slate-600 hover:border-emerald-800 hover:text-emerald-800'}`} onClick={() => setFilter('popular')}>🌟 Popular</button>
          <button className={`px-5 py-2 rounded-full font-headline font-bold text-sm border-2 transition-all ${filter === 'scenic' ? 'bg-emerald-800 border-emerald-800 text-white' : 'border-slate-300 bg-white text-slate-600 hover:border-emerald-800 hover:text-emerald-800'}`} onClick={() => setFilter('scenic')}>🏔️ Scenic</button>
          <button className={`px-5 py-2 rounded-full font-headline font-bold text-sm border-2 transition-all ${filter === 'spiritual' ? 'bg-emerald-800 border-emerald-800 text-white' : 'border-slate-300 bg-white text-slate-600 hover:border-emerald-800 hover:text-emerald-800'}`} onClick={() => setFilter('spiritual')}>🛕 Spiritual</button>
          <button className={`px-5 py-2 rounded-full font-headline font-bold text-sm border-2 transition-all ${filter === 'adventure' ? 'bg-emerald-800 border-emerald-800 text-white' : 'border-slate-300 bg-white text-slate-600 hover:border-emerald-800 hover:text-emerald-800'}`} onClick={() => setFilter('adventure')}>🧗 Adventure</button>
        </div>
      </section>
          ...
          {filteredRoutes.map((route) => (
            <div key={route.id} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-slate-100">
              <div className={`h-48 bg-gradient-to-br ${route.bgClass} flex items-end p-6 relative overflow-hidden`}>
                <div className="absolute inset-0 opacity-20 text-[8rem] flex items-center justify-center">{route.icon}</div>
              ...
              <div className="p-6 space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="flex items-center gap-1 text-tertiary"><span className="material-symbols-outlined text-base">straighten</span> {route.distance}</span>
                  <span className="flex items-center gap-1 text-tertiary"><span className="material-symbols-outlined text-base">schedule</span> {route.time}</span>
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setSelectedRouteId(route.id)} className="flex-1 text-center border-2 border-primary text-primary rounded-xl py-3 font-headline font-bold hover:bg-emerald-50/50 transition-colors flex items-center justify-center gap-1 text-xs sm:text-sm">
                    <span className="material-symbols-outlined text-base">map</span> Map & Stops
                  </button>
                  <Link href={`/?pickup=${route.pickup}&drop=${route.drop}&step=1`} className="flex-1 text-center bg-primary text-white rounded-xl py-3 font-headline font-bold hover:bg-emerald-700 transition-colors flex items-center justify-center gap-1 text-xs sm:text-sm shadow-md hover:shadow-lg">
                    <span className="material-symbols-outlined text-base">bolt</span> Book Ride
                  </Link>
                </div>
              </div>
            </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-screen-xl mx-auto bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-700 rounded-[40px] p-12 text-center text-white">
          ...
          <Link href="/contact" className="inline-flex items-center gap-3 bg-white text-emerald-900 px-10 py-4 rounded-2xl font-headline font-extrabold text-lg hover:bg-green-50 transition-all hover:-translate-y-1 shadow-xl duration-300">
